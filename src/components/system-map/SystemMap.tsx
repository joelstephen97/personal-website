...
import { MAP, arcPath, bezelTicks, nodePosition, shortestRotation } from "./geometry";
...
export function SystemMap({
  ...
  const markerBase = useMotionValue(0);
  ...
  useEffect(() => {
    if (!hot) return;
    const angle = angleById.get(hot);
    if (angle == null) return;
    const target = shortestRotation(markerBase.get(), angle + 90);
    markerBase.set(target);
    if (reduced) markerSpring.jump(target);
    // angleById is rebuilt every render from the `domains` prop; only `hot`
    // (and the motion values, which are stable) should retrigger this.
  ...
  return (
    <div className={cn("relative", variant === "compact" && "h-[280px]", className)}>
      ...
      >
        ...
        <m.g
          style={{ transformOrigin: `${MAP.cx}px ${MAP.cy}px`, transformBox: "fill-box" }}
          initial={animateOnMount ? { scale: 0.6, opacity: 0 } : false}
          animate={{ scale: 1, opacity: 1 }}
