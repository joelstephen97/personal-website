<template>
  <div class="min-h-screen bg-[rgb(var(--bg))] px-6 py-12">
    <div class="max-w-6xl mx-auto">
      ...
      <!-- Toolbar -->
      <div
        class="glass-solid rounded-2xl px-4 py-2 mb-4 flex flex-wrap gap-1 no-print"
      >
        <button
...
function wrapSelection(before: string, after: string) {
  ...
  source.value =
    source.value.slice(0, start) +
    before +
    selected +
    after +
    source.value.slice(end);
  nextTick(() => {
    el.focus();
...
function insertAtCursor(text: string) {
  ...
  const start = el.selectionStart;
  source.value =
    source.value.slice(0, start) + text + source.value.slice(start);
  nextTick(() => {
    el.focus();
...
function parseInline(text: string): string {
  ...
  result = result.replace(
    /!\[([^\]]*)\]\(([^)]+)\)/g,
    (_, alt, src) =>
      `<img src="${sanitizeUrl(src)}" alt="${escapeHtml(alt)}" style="max-width:100%;border-radius:8px" />`,
  );
  result = result.replace(
    /\[([^\]]+)\]\(([^)]+)\)/g,
    (_, text, url) =>
      `<a href="${escapeHtml(sanitizeUrl(url))}" target="_blank" rel="noopener noreferrer">${text}</a>`,
  );
  return result;
}
...
const rendered = computed(() => {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("> ") || line === ">") {
      ...
      html.push(
        `<blockquote>${quoteLines.map(parseInline).join("<br>")}</blockquote>`,
      );
      continue;
    }
