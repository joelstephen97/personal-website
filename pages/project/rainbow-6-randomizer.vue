<template>
  ...
  >
    <!-- Overlay Div -->
    <div
      :class="colorMode.value === 'dark' ? 'bg-black opacity-50' : 'bg-white opacity-50'"
      class="fixed inset-0"
      aria-hidden="true"
    ></div>
    ...
    <div class="relative z-10 flex flex-col items-center w-full max-w-4xl">
      ...
      <div class="flex flex-col items-center mb-8 w-full">
        ...
        <div v-if="selectedAttackers.length" class="w-full">
          ...
          <div class=" p-2 flex flex-row flex-nowrap items-center justify-evenly gap-4 w-[fit] overflow-x-auto">
            <div
              class="flex flex-col items-center p-2 rounded max-h-fit"
              :class="colorMode.value === 'dark' ? 'bg-gray-900 shadow-[0_0_10px_white]' : ' bg-slate-100 shadow-[0_0_10px_black]'"
            ...
            >
              ...
              <p class="text-center font-bold mb-1 uppercase">{{ assignment.operator.name }}</p>
              <p class="text-center font-bold text-sm mb-1 text-ellipsis overflow-hidden w-28">{{ assignment.name }}</p>
              <button @click="rerandomizeAttacker(index)" class="text-blue-500 hover:underline text-md">Re-randomize</button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div v-if="selectedDefenders.length" class="w-full">
          ...
          <div class=" p-2 flex flex-row flex-nowrap items-center justify-evenly gap-4 w-[fit] overflow-x-auto">
            ...
            >
              ...
              <p class="text-center font-bold mb-1 uppercase">{{ assignment.operator.name }}</p>
              <p class="text-center font-bold text-sm mb-1 text-ellipsis overflow-hidden w-28">{{ assignment.name }}</p>
              <button @click="rerandomizeDefender(index)" class="text-blue-500 hover:underline text-md">Re-randomize</button>
            </div>
          </div>
        </div>
      </div>
