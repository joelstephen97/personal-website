    ...
    <div class="max-w-3xl mx-auto">
      ...
      <div class="glass-solid rounded-2xl p-6 mb-6">
        <div class="flex flex-wrap items-end gap-4">
          <div>
            <label class="text-xs text-[rgb(var(--foreground-muted))] uppercase tracking-wide block mb-2">Base Color</label>
            <div class="flex items-center gap-3">
              <input v-model="hex" type="color" class="w-12 h-12 rounded-xl border-0 cursor-pointer" />
              <input
                v-model="hex"
                type="text"
          ...
          <div>
            <label class="text-xs text-[rgb(var(--foreground-muted))] uppercase tracking-wide block mb-2">Harmony</label>
            <select v-model="harmony">
              <option v-for="h in harmonies" :key="h.key" :value="h.key">{{ h.label }}</option>
            </select>
          </div>
          <button
      ...
      <div class="glass-solid rounded-2xl p-6 mb-6">
        <h3 class="text-sm font-semibold text-[rgb(var(--foreground))] mb-4">Generated Palette</h3>
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
          ...
          >
            ...
            />
            <p class="text-xs font-mono text-[rgb(var(--foreground))] text-center">{{ color.hex }}</p>
            <p class="text-[10px] text-[rgb(var(--foreground-muted))] text-center">{{ color.rgb }}</p>
      ...
      <div class="glass-solid rounded-2xl p-6 mb-6">
        <h3 class="text-sm font-semibold text-[rgb(var(--foreground))] mb-4">Tints &amp; Shades</h3>
        <div class="flex gap-1 rounded-xl overflow-hidden">
          <div
            ...
            @click="copyColor(c)"
          >
            <span class="absolute inset-0 flex items-center justify-center text-[8px] font-mono opacity-0 group-hover:opacity-100 transition-opacity"
              :style="{ color: i < 5 ? 'white' : 'black' }">{{ c }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="grid sm:grid-cols-2 gap-6 mb-6">
        ...
        <div class="glass-solid rounded-2xl p-6">
          <h3 class="text-sm font-semibold text-[rgb(var(--foreground))] mb-3">Contrast on Black</h3>
          <div class="flex items-center gap-3">
...
const baseHsl = computed(() => { const [r, g, b] = hexToRgb(hex.value); return rgbToHsl(r, g, b); });

interface PaletteColor { hex: string; label: string; rgb: string; hslLabel: string; }
function makeColor(h: number, s: number, l: number, label: string): PaletteColor {
  const hx = hslToHex(h, s, l);
  const [r, g, b] = hexToRgb(hx);
  return { hex: hx, label, rgb: `rgb(${r}, ${g}, ${b})`, hslLabel: `hsl(${Math.round(h)}, ${Math.round(s)}%, ${Math.round(l)}%)` };
}
...
const palette = computed(() => {
  ...
  switch (harmony.value) {
    case "complementary":
      return [makeColor(h, s, l, "Base"), makeColor(h + 180, s, l, "Complement")];
    case "analogous":
      return [makeColor(h - 30, s, l, "-30°"), makeColor(h, s, l, "Base"), makeColor(h + 30, s, l, "+30°")];
    case "triadic":
      return [makeColor(h, s, l, "Base"), makeColor(h + 120, s, l, "+120°"), makeColor(h + 240, s, l, "+240°")];
    case "split":
      return [makeColor(h, s, l, "Base"), makeColor(h + 150, s, l, "+150°"), makeColor(h + 210, s, l, "+210°")];
    case "tetradic":
      return [makeColor(h, s, l, "Base"), makeColor(h + 90, s, l, "+90°"), makeColor(h + 180, s, l, "+180°"), makeColor(h + 270, s, l, "+270°")];
    case "monochromatic":
