  ...
  <div class="min-h-screen bg-[rgb(var(--bg))] px-6 py-12">
    <div class="max-w-3xl mx-auto">
      ...
      <div class="glass-solid rounded-2xl p-6 mb-6">
        ...
        <p v-if="regexError" class="text-accent text-sm mb-4">
          {{ regexError }}
        </p>
        ...
        >
          <span
            ...
            :key="i"
            :class="
              seg.match ? matchColors[seg.groupIdx % matchColors.length] : ''
            "
            >{{ seg.text }}</span
          >
          <span
      ...
      <div class="grid sm:grid-cols-2 gap-6 mb-6">
        <div class="glass-solid rounded-2xl p-6">
          ...
          >
            ...
            <p v-if="execTime !== null">
              Time:
              <strong class="text-accent">{{ execTime.toFixed(3) }}ms</strong>
            </p>
            ...
            <div v-if="matchPositions.length" class="mt-2 space-y-1">
              <p v-for="(pos, i) in matchPositions" :key="i" class="text-xs">
...
const commonPatterns = [
  {
    label: "Email",
    pattern: "(\\w+)@(\\w+\\.\\w+)",
    sample: "Contact hello@example.com or support@test.org",
  },
  {
    label: "URL",
    pattern: "https?://[^\\s]+",
    sample: "Visit https://example.com for more",
  },
  {
    label: "Phone",
    pattern: "\\d{3}[-.]?\\d{3}[-.]?\\d{4}",
    sample: "Call 555-123-4567 or 555.123.4567",
  },
  {
    label: "Date",
    pattern: "\\d{4}-\\d{2}-\\d{2}",
    sample: "Event on 2024-03-15",
  },
  {
    label: "IPv4",
...
watch([regex, testStr], updateMatches, { immediate: true });
...
const segments = computed((): Segment[] => {
  ...
  if (!matches.value.length)
    return [{ text: testStr.value, match: false, groupIdx: 0 }];
  ...
  for (let i = 0; i < matches.value.length; i++) {
    ...
    if (m.index > lastIdx)
      result.push({
        text: testStr.value.slice(lastIdx, m.index),
        match: false,
        groupIdx: 0,
      });
  ...
  if (lastIdx < testStr.value.length)
    result.push({
      text: testStr.value.slice(lastIdx),
      match: false,
      groupIdx: 0,
    });
  return result;
  ...
  while (i < p.length) {
    if (p[i] === "\\" && p[i + 1]) {
      ...
      parts.push(
        prev +
          " (" +
          (p[i] === "*" ? "0 or more" : p[i] === "+" ? "1 or more" : "0 or 1") +
          ")",
      );
      i++;
    } else if (p[i] === "[") {
